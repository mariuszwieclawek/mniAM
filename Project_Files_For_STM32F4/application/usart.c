...
bool USART_PutChar(char c){
	//TODO
	__disable_irq();
	if(RingBuffer_PutChar(&USART_RingBuffer_Tx,c)){ // send char to ring buffer tx
		__USART_ENABLE_IT(&UartHandle, USART_IT_TXE); // enable interrupt TXE
		__enable_irq();
		return true;
	}
	else{
...
size_t USART_WriteData(const void *data, size_t dataSize){
	...
	for(i = 0; i<dataSize; i++){
		USART_PutChar(((const char*)data)[i]);
	}
	__enable_irq();
	return i;
}
...
size_t USART_WriteString(const char *string){
	//TODO
	//printf("dlugosc:%d",strlen(string));
	__disable_irq();
	USART_WriteData(string,strlen(string));
	__enable_irq();
...
void USARTx_IRQHandler(void){
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
	    // the TXE interrupt is enabled
			...
			char txtest = 't';
			if(RingBuffer_GetChar(&USART_RingBuffer_Tx,&txtest) == true)
				USARTx->DR = txtest; // send char to usart
			else
				__USART_DISABLE_IT(&UartHandle, USART_IT_TXE); // disable the interrupt source
